    ...
    static AuthModule create(AuthType type) {
        switch (type) {
            case AuthType::LOGIN_PASSWORD:
                return {
                    std::make_unique<LoginPasswordAuth>(),
                    std::make_unique<LoginPasswordUI>()
                };
            case AuthType::PHONE:
                return {
                    std::make_unique<PhoneNumberAuth>(),
                    std::make_unique<PhoneNumberUI>()
                };
            case AuthType::QR:
                return {
                    std::make_unique<QRCodeAuth>(),
                    std::make_unique<QRCodeUI>()
                };
        }
